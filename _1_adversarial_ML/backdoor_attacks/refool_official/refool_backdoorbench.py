...
def torch_normalize(torch_tensor: torch.tensor):
    min_ = torch.min(torch_tensor)
    max_ = torch.max(torch_tensor)
    return (torch_tensor-min_)/(max_-min_) if max_>min_ else torch_tensor
...
class AddTriggerMixin(object):
    ...
    Args:
        ...
        normalize_images (bool): whether or not to normalize images
    """
    def __init__(self, total_num, reflection_cadidates, max_image_size=560, ghost_rate=0.49, alpha_b=-1., offset=(0, 0), sigma=-1, ghost_alpha=-1., normalize_images: bool=True):
        super(AddTriggerMixin,self).__init__()
        self.reflection_candidates = 255 * normalize(reflection_cadidates)
    ...
    def _add_trigger(self, sample, index):
        ...
        min_ = torch.min(sample).numpy(); max_ = torch.max(sample).numpy()
        sample = 255 * torch_normalize(sample)
        ...
        blended = blended.astype(np.float32) / 255; blended = blended*(max_-min_)+min_
