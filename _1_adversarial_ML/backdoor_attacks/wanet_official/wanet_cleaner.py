...
def train_toy(train_dl, noise_grid, identity_grid, opt: Parameters):
    
    rate_bd = 1
    
    all_inputs, all_bd_inputs = [], []
    ...
    return torch.cat(all_inputs, dim=0), torch.cat(all_bd_inputs, dim=0)


def eval_toy(test_dl, noise_grid, identity_grid, opt: Parameters):
    total_sample = 0
    
    all_inputs, all_bd_test_inputs = [], []
    ...
    return torch.cat(all_inputs, dim=0), torch.cat(all_bd_test_inputs, dim=0)


def train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, epoch, opt: Parameters, verbose: bool=True, pre_str: str='', color: str=None):
    # print(" Train:")
    netC.train()
    rate_bd = opt.pc
    total_loss_ce = total_sample = 0
    total_clean = total_bd = total_cross = 0
    total_clean_correct = total_bd_correct = total_cross_correct = 0
...
def eval(netC, test_dl, noise_grid, identity_grid, best_clean_acc, best_bd_acc, best_cross_acc, opt: Parameters, verbose: bool=True, pre_str: str='', color: str=None):
    netC.eval()
    total_sample = total_clean_correct = total_bd_correct = total_cross_correct = total_ae_loss = 0
...
def main(torch_model: Torch_Model):
    ...
    for epoch in range(opt.n_iters):
        ...
        poisoned_test = train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, epoch, opt)
        best_clean_acc, best_bd_acc, best_cross_acc, bd_test_inputs = eval(netC, optimizerC, schedulerC, test_dl, 
            noise_grid, identity_grid, best_clean_acc, best_bd_acc, best_cross_acc, epoch, opt)
