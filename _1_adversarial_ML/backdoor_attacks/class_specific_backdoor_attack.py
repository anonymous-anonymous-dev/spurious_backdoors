...
class Class_Specific_Backdoor(Simple_Backdoor):
    ...
    def __init__(
        ...
        msg_ = f'Target class ({self.targets[0]}) is same as victim class ({self.backdoor_configuration['victim_class']}), which does not makes sense.'
    ...
    def poison(self, x, y, **kwargs):
        ...
        return torch.clamp(x+self.triggers[0]*(max_value-min_value), min_value, max_value), return_target
