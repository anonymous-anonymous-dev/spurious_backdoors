...
class PCA_Analyzer:
    ...
    def __init__(
        ...
        x, y, ac_, 
        epsilon=0.5, 
        mode='multi', sample_subset: int=0.1, iterations: int=100,
        ...
        if self.mode == 'one':
            ...
            self.sample_subset = int(sample_subset*len(x))
            self.replace = True
            self.prepare_analyzer_smooth(x, y)
    ...
    def prepare_analyzer_smooth(self, x, y):
        ...
        for i in range(self.iterations):
            ...
            pc_p = pca_analyzer.transform(p_activations).reshape(len(xp), -1)
            # pc_max = np.max(pc)
            # pc_min = np.min(pc)
        ...
        self.pcs_ = np.median(self.pcs_s, axis=0)
        self.pcs_u_min = np.min(self.pcs_, axis=0, keepdims=True)
        self.pcs_u_max = np.max(self.pcs_, axis=0, keepdims=True)
        self.pcs_ = (self.pcs_-self.pcs_u_min)/(self.pcs_u_max-self.pcs_u_min)
        self.pcs_u_median = np.median(self.pcs_, axis=0, keepdims=True)
        self.pcs_ = self.pcs_ - self.pcs_u_median
        
        self.scores_ = np.median(self.scores_s, axis=0)
        self.scores_u_min = np.min(self.scores_)
        self.scores_u_max = np.max(self.scores_)
        self.scores_ = (self.scores_-self.scores_u_min)/(self.scores_u_max-self.scores_u_min)
        
        metric_scores = np.append(self.pcs_, self.scores_.reshape(-1, 1), axis=1)
        
        # y_pred = get_outputs(self.model.model, prepare_dataloader_from_numpy(x, y, batch_size=self.batch_size), return_numpy=True)
        # y_pred = self.np_softmax(y_pred)
        # confidences = y_pred[:, self.target_class]
        # confidences = confidences.reshape(len(x), 1)
        # metric_scores = np.append(metric_scores, confidences, axis=1)
        ...
        self.poisoning_score = np.mean(self.scores_[np.where(self.clusterer.labels_==self.bad_label)])*(np.max(label_lengths))
    ...
    def analyze(self, x, y):
        ...
        for i in range(self.iterations):
            ...
            pc_p = self.pca_analyzers[i].transform(p_activations).reshape(len(xp), -1)
        ...
        pcs_s = (pcs_s - self.pcs_u_min) / (self.pcs_u_max - self.pcs_u_min)
        pcs_s = pcs_s - self.pcs_u_median
        
        scores_s = np.mean(scores_s, axis=0)
        scores_s = (scores_s - self.scores_u_min) / (self.scores_u_max - self.scores_u_min)
    ...
    def forward(self, x, y_out):
        ...
        y_random = np.random.normal(0, 10, size=y_out.shape)
