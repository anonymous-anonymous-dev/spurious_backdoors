...
class My_Clusterer:
    ...
    def get_ellipse(self):
        return Ellipse(xy=(self.center_x, self.center_y), width=self.width, height=self.height, angle=self.angle_degrees, edgecolor='green', facecolor='none', linewidth=2)
...
class PCA_Analyzer:
    ...
    def __init__(
        ...
        mode='multi', sample_subset: int=30, iterations: int=100, std: float=0.2,
        ...
        #     metric='l2', algorithm='auto', min_cluster_size=len(x)//2, min_samples=1, allow_single_cluster=True,
        #     cluster_selection_epsilon=0.03
        # )
        # self.clusterer = SpectralClustering(n_clusters=2, affinity='precomputed')
    ...
    def prepare_analyzer_smooth(self, x, y):
        ...
        # self.pcs_ = np.mean(self.pca_stats.pcs_s, axis=0)
        # # print(np.array(self.pca_stats.scores_dict['values']).shape)
        # # self.scores_ = np.mean(self.pca_stats.scores_dict['values'], axis=0)
        # self.scores_ = np.mean(self.pca_stats.scores_, axis=0)
        ...
        print('\n', labels)
        print(self.good_label, self.bad_label)
        ...
        # self.poisoning_score = np.mean(self.pcs_[bad_l, 1]) - np.mean(self.pcs_[good_l, 1])
        ...
        # self.ref_pcs, self.ref_scores, _ = super().analyze(xr, yr)
        self.custom_clusterer.fit(self.limited_pa_)
        self.good_label = 0; self.bad_label = 1;
        # self.poisoning_score = self.threshold + 1
        
        # bad_indices = max(np.sum(bad_l) // 2, 1); good_indices = max(np.sum(good_l) // 2, 1)
        # sorted_adv_score_bad = np.sort(self.pcs_[bad_l, 1]); sorted_adv_score_good = np.sort(self.pcs_[good_l, 1]); 
        # sorted_spu_score_bad = np.sort(self.scores_[bad_l]); sorted_spu_score_good = np.sort(self.scores_[good_l]); 
        # diff_adv_score = np.median(sorted_adv_score_bad)# - np.median(sorted_adv_score_good[-good_indices:])) #/ np.max(sorted_adv_score_good)
        # diff_spu_score = np.median(sorted_spu_score_bad)# - np.median(sorted_spu_score_good[-good_indices:])) #/ np.max(sorted_spu_score_good)
        # weightage = np.exp(-diff_spu_score)
        # self.poisoning_score = diff_spu_score + weightage*diff_adv_score
        # self.poisoning_score = self.poisoning_score * max_l
        # print(colored('\n***********************************************', 'light_green'))
        # print(colored(f'***** Diff Poison Score: {diff_spu_score:.5f} ***********', 'light_green'))
        # print(colored(f'***** Diff Adv Score: {diff_adv_score:.5f} ***********', 'light_green'))
        # print(colored(f'***** Weightage: {weightage:.5f} ***********', 'light_green'))
    ...
    def analyze(self, x, y):
        ...
        # metric = normalize(np.append(self.metric_scores, metric_scores, axis=0))
        # metric = 1 + self.pairwise_cosine_similarity_torch(metric)
        # self.clusterer.fit(metric)
        # _label = deepcopy(self.clusterer.labels_)
        # good_label = np.median(_label[:len(self.metric_scores)]).astype('int')
        # all_labels = np.zeros_like(_label)
        # all_labels[_label!=good_label] = 1
        # label = all_labels[len(self.metric_scores):]
        label = self.clusterer.predict(metric_scores)
        # if self.poisoning_score 
        label[(label==self.bad_label) & (np.abs(pcs_s[:,1])<0.18) & (np.abs(pcs_s[:,0])<0.4)] = self.good_label
        
        if self.custom_clusterer.configured:
            label = self.custom_clusterer.predict(pcs_s[:, 0], pcs_s[:, 1])
            scores_s = label * scores_s
            label[label>=0.5] = 1
            label[label<0.5] = 0
