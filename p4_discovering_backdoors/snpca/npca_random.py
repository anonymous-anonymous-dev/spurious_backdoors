...
class NPCA_Random(NPCA_Custom_with_Masking):
    ...
    def get_data_subset_personal(self, data_in, class_id, bs=128, num_workers=0, **kwargs):
        
        self.print_out(colored('using random indices...', 'light_red'))
        # _poison_in = np.random.choice(data_in.__len__(), min(10000, data_in.__len__()), replace=False)
        _poison_in = np.random.choice(data_in.__len__(), 10000, replace=True)
        
        in_subset = Client_SubDataset(data_in, _poison_in)
    ...
    def prepare_analyzer(self, *args, mode: str='one', threshold: float=0, **kwargs):
        ...
        transformed_loader = prepare_dataloader_from_numpy(_xq[y==self.target_class], y[y==self.target_class], batch_size=self.data_loader.batch_size)
        # self.torch_model.data.reset_transforms()
        return PCA_Analyzer_Random(
            *args, 
            threshold=0, epsilon=self.configuration['adversarial_epsilon'],
    ...
    def prepare_masked_data(self):
        ...
        # assert False
        # show_image_grid(normalize(perturbed_x), n_rows=5, n_cols=5, channels_first=True);
        ...
        pred_y = get_outputs(self.model.model, prepare_dataloader_from_numpy(perturbed_x, y_targeted, batch_size=self.batch_size), return_numpy=True)
        pred_y = np.argmax(pred_y, axis=1)
        successful_indices = (pred_y == self.target_class)
        if np.mean(successful_indices)>0:
            ...
            self.patches = patch_perturbations.copy()
