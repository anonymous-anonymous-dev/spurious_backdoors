...
from .counterfactual import generate_feature_counterfactuals
...
class  ActivationSpace():
    ...
    def fit(self, eigvec_scale=True, only_act=False, activations_address=None):
        if self._train_loader is None:
            raise Exception('Missing train_loader. Use AcitvationSpace.load() or init with train_loader.')
        
        # get activations
        print('Computing activations\n')
        self.activations_train = self._compute_training_activations(only_act)# if activations_address is None else torch.from_numpy(np.load(activations_address))
        ...
        if only_act:
            ...
            print(self.activations_train.device, self.pca_mean.device)
    ...
    def _compute_training_activations(self, only_act=False):
        ...
        for i, (imgs, _) in enumerate(self._train_loader):
            ...
            if not only_act:
                ...
                if act_train is None:
                    ...
                    if self.confidences_train is None:
                        self.confidences_train = prob; self.predictions_train = pred
                    else:
                ...
                print(f'\rProgress [{100*(i+1)/len(self._train_loader):.2f} %]', end='')
        return act_train
    ...
    def _pca_transform(self, activations, eigvec_scale=True):
        ...
        act = activations.to(self.device) - self.pca_mean.to(self.device)
