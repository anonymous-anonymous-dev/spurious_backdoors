...
def visualize_components(class_idx, device, batchsize, n_components=10, load_act_space=False, load_conf=False):
    ...
    if model_id == "robust_resnet":
        ...
        last_layer = list(list(model.model.model.children())[-1].children())[-1]
        gradcam_layers = list(list(model.model.model.children())[-1].children())[-4] # [model.model.model.layer4[-1]]
    else:
        last_layer = None
        gradcam_layers = None
...
def visualize_components_sub(_model, _last_layer, class_idx, device, batchsize, n_components=10, load_act_space=False, load_conf=False):
    config = {
        'target_class':class_idx,
        'loss':'obj_full',
        'model':'robust_resnet',
    ...
    if config['load_act_space']:
        ...
        load_path = None
    act_space = get_latent_space(_model, _last_layer, config['target_class'], config['device'], config['batch_size'], config['n_workers'], act_space_save_dir, load_path)
    ...
    for i, pc_idx in enumerate(top_idcs):
        ...
        ax[1,0].imshow(cfs[pc_idx].permute(1,2,0))
        ax[1,0].set_title(f'Prediction: {data.imagenet_label2class[pred[pc_idx].item()]}\nConf. {target_name}: {prob[pc_idx, config["target_class"]]:.4f}\nAlpha: {alpha[pc_idx, pc_idx]:.4f}\nCriterion: {criterion[pc_idx]:.4f}', fontsize=fontsize)
        ax[1,0].set_ylabel(f'PC {pc_idx}', fontsize=fontsize)
        ax[1,0].tick_params(axis='both', which='both', bottom=False, top=False, right=False, left=False, labelbottom=False, labeltop=False, labelright=False, labelleft=False)
        ...
        for j, (max_img, max_idx) in enumerate(zip(max_imgs[pc_idx], max_idcs[pc_idx])):
            ...
            # Max. activating training image
            ax[1,j+1].imshow(max_img.permute(1,2,0))
            ax[1,j+1].set_title(f'Max. act. train - {j}\nConf. {target_name}: {act_space.confidences_train[max_idx, config["target_class"]]:.4f}\nAlpha: {act_space.pca_train[max_idx, pc_idx]:.4f}', fontsize=fontsize)
            ax[1,j+1].axis('off')
