...
class ViT(nn.Module):
    ...
    def forward(self, images):
        ...
        out = self.fc2(self.classifier(out))
        return out



def ViT16_CIFAR10(patch_height: int=4, patch_width: int=None):
    patch_width = patch_height if patch_width is None else patch_width
    return ViT(
        patch_height=patch_height, patch_width=patch_width, embedding_dims = 768, dropout = 0.1, heads = 4, num_layers = 4, forward_expansion = 4,
        max_len = int((32*32)/(patch_height*patch_width)), layer_norm_eps = 1e-5,
        num_classes = 10
    )
...
def ViT16_CIFAR100():
    return ViT(
        patch_height = 16, patch_width = 16, embedding_dims = 768, dropout = 0.1, heads = 4, num_layers = 4, forward_expansion = 4,
        max_len = int((32*32)/(16*16)), layer_norm_eps = 1e-5,
        num_classes = 100
    )
