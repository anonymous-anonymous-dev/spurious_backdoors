...
class Torch_Model(Torch_Model_Plugin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(data, model_configuration, path=path)
    ...
    def train(
        ...
        **kwargs
        ...
        for epoch in range(start_epoch, epochs+1):
            ...
            if validate:
                test_loss, test_acc, _ = self.test_shot(test_loader, verbose=verbose, pre_str=train_str)
    ...
    def load_weights(self, name, load_optimizer=True):
        ...
        if os.path.isfile(name+'.pth'):
            ...
            if load_optimizer and os.path.isfile(name+'_optimizer.pth'):
                self.optimizer.load_state_dict(torch.load(name+'_optimizer.pth', map_location=torch.device(self.device)))
